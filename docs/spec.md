...
- 不裁切内容
- 不允许为适配布局而改变窗口逻辑宽高比
- all/multi-workspace overview 中单个 preview 的最终 scale 不得超过 `max_preview_scale`
- active-workspace overview 中单个 preview 的最终 scale 不得超过 `workspace_overview_max_preview_scale`
...
Thumbnail 引擎 (`layout_engine = thumbnail`)：

- 窗口按 workspace 分组，每个 workspace 形成一个长方形缩略图卡片
- 卡片长宽比与 overview 区域一致（monitor-shaped）
- 窗口在其所属卡内保持精确的原始相对位置和缩放比例
...
- 只接管 Hyprland 官方 gesture 语法里的 `dispatcher, hymission:toggle,...` / `dispatcher, hymission:open,...` / `dispatcher, hymission:scroll,...`；同时兼容 Hyprland 官方 `scrollMove`
- 推荐写法：`gesture = 4, vertical, dispatcher, hymission:toggle,forceall`
- niri mode 推荐写法：`gesture = 3, horizontal, dispatcher, hymission:scroll,layout`、`gesture = 3, vertical, workspace`；也可用 `gesture = 3, horizontal, scrollMove`
...
- `up` / `down` / `left` / `right` 继续走 Hyprland 默认 gesture 语义；`swipe` 只由 `hymission:scroll` 接管
...
- `scrollMove` 的平移、惯性投影、snap 行为由 Hyprland 官方实现负责；overview 可见且 `niri_mode = 1` 时，hymission 在官方 gesture 更新后刷新 niri overview 的 preview geometry
...
- 使用 ANSI 主键区的 36 个物理字母数字键；几何位置与按键命中由物理 keycode 决定，不受当前 XKB layout、Shift 或 Caps Lock 影响
- 标签字符自动读取活动键盘的当前 XKB layout，并固定使用无 Shift/Caps Lock 的基础层；运行中切换 layout 后标签随之更新，但窗口映射保持稳定
...
- `small_window_boost`
- `max_preview_scale`
- `workspace_overview_max_preview_scale`
- `min_slot_scale`
- `layout_engine`
...
- `niri_mode = 1` 不自动改变 `workspace_strip_anchor` 或 `workspace_strip_thickness`；strip 仍由这些配置决定边缘小条位置和厚度，主 overview 内容区仍给 strip 留出空间
